#!/usr/bin/env bash
# check-careful.sh — PreToolUse hook for safety skill (careful mode)
...
if printf '%s' "$CMD" | grep -qE 'rm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+|--recursive\s+)' 2>/dev/null; then
  SAFE_ONLY=true
  # Strip everything up to and including "rm" + all flags to get just targets
  RM_ARGS=$(printf '%s' "$CMD" | sed -E 's/^.*rm[[:space:]]+//' | sed -E 's/-[a-zA-Z]+[[:space:]]*//g;s/--[a-z]+[[:space:]]*//g' | tr -s ' ' | sed 's/^ //')
